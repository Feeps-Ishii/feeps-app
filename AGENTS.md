...
> ⚠️ **詳細ルールは [../../AGENTS.md](../../AGENTS.md)（`C:\Feeps_LMS\AGENTS.md`）を参照すること。** 実装前報告フォーマット・自律実行ルール(commit/deployの4点セット)・ドキュメント更新義務など、全リポジトリ共通のルールはそちらに集約されており、**本ファイル単体では不完全**（Frontendリポジトリ固有の補足情報のみ）。もしこのディレクトリ内からエージェントを起動していて`../../AGENTS.md`が自動的には読み込まれていない場合、必ず明示的に開いて読むこと。
