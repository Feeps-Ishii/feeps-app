...
import { Card, Badge, Btn, Avatar, Stat, SectionHead, Field, Seg, T, PageHeader, EmptyState as CommonEmptyState, SkeletonRows, SkeletonCards, Modal, MonthPicker } from "../../components/common";
import {
  QBANK
...
import {
  ...
  GitBranch, Briefcase, Gauge, RefreshCw, Calendar
} from "lucide-react";
  ...
  return (
    <div>
      <PageHeader
        ...
        cta={{ label: "参加コースを見る", icon: BookOpen, onClick: () => go("courses") }}
      />
      ...
      <Card className="mt-6 overflow-hidden">
        ...
        {clientLoading ? <SkeletonRows />
          ...
          : <div className="divide-y" style={{ borderColor: T.border }}>{joinedClientCourses.map(c => <div key={c.courseId} className="flex flex-wrap items-center justify-between gap-3 p-4"><div className="min-w-0"><div className="truncate font-bold" style={{ color: T.textPrimary }}>{c.name}</div><div className="mt-1 text-xs" style={{ color: T.textMuted }}>{kindLabel(c.type || c.kind)}</div></div><Btn size="sm" kind="ghost" icon={ChevronRight} onClick={() => { setActiveCourseId(c.courseId); go?.("courses"); }}>コースを見る</Btn></div>)}</div>}
      </Card>
      <Card className="mt-6 p-5">
        ...
        <div className="space-y-2">{clientLoading ? <SkeletonRows rows={3} /> : clientTrainees.slice(0, 6).map(t => (
