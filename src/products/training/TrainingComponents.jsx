...
// 2026-08-24: 受講者ホームの「状況ボード」（承認モック: mock/training-home の案A）。
//
// 以前は画面の3割をヒーローが使って伝えているのは1行、その下は同じ形のリンク4本で、
// 数字が1つも無かった。受講者が知りたいのは「今日どうすればいいか」と
// 「自分が何を残しているか」の2つなので、それを最初の1枚へ入れる。
// 研修日／研修がない日で**同じ骨格**を使い、中身だけ入れ替える。
function StatusBoardStat({ tone = "neutral", value, unit, label }) {
  const c = {
    warn: { bg: PRISM.warnSubtle, fg: PRISM.warn },
    accent: { bg: PRISM.surface, fg: PRISM.accent },
    ok: { bg: PRISM.surface, fg: PRISM.ok },
    neutral: { bg: PRISM.surface, fg: PRISM.ink },
  }[tone] || { bg: PRISM.surface, fg: PRISM.ink };
  return (
    <div className="rounded-[14px] px-[15px] py-3.5" style={{ background: c.bg }}>
      <div className="text-[24px] font-extrabold leading-none tabular-nums" style={{ color: c.fg, letterSpacing: "-0.03em" }}>
        {value}{unit && <span className="text-[13px] font-bold" style={{ color: PRISM.mut }}>{unit}</span>}
      </div>
      <div className="mt-1 text-[11.5px] leading-snug" style={{ color: PRISM.sub }}>{label}</div>
    </div>
  );
}
...
function formatTrainingDate(iso) {
  ...
  return `${d.getMonth() + 1}月${d.getDate()}日（${TH_WEEKDAY[d.getDay()]}）`;
}
...
function TraineeHome({ go, goProduct, goSub, done, taskDataState, onTaskRetry, taskSaveState, toggle, goals }) {
  ...
  // 研修管理Home刷新（モード分離Step2）: ヒーロー文言・CTAは日/非日で分ける。既存の
  // pending/unavailableタスク判定ロジックはそのまま踏襲し、Eラーニング導線のみ除去する。
  const thHeroKicker = `${thPrimaryCourse?.name || "FEEPS ONE"} ・ ${thToday}`;
  ...
  const thHeroDescription = thLoading
    ...
    : thNoTrainingToday
      ? "勤怠と日報の入力は不要です。研修資料の見返しや次回の予習ができます。"
      : thPendingTasks.length > 0
  ...
  const thHeroIllustration = thNoTrainingToday ? <TraineeOffDayIllustration /> : <TraineeTrainingDayIllustration />;

  // --- 状況ボード（2026-08-24 案A）で使う値 ---
  const thNextTrainingDay = thHome.dashboard?.nextTrainingDay || null;
  const thNextTrainingDayAvailable = thHome.dashboard?.availability?.nextTrainingDay !== false;
  const thUnsubmittedTestCount = Number(thTrainingTests?.unsubmitted || 0);
  const thSubmittedReportCount = thHome.reports.filter(r => r && r.date).length;
  // 研修がない日は「今日の日報」が存在しないので、**直近の日報**のコメントを見る。
  // 今日だけを見ていると、研修がない日には講師コメントが一生出てこない。
  ...
  const thRequiredToday = thTaskCards.map(card => ({
    key: card.key,
    label: card.key === "attendance" ? "勤怠" : "日報",
    done: card.done && !card.unavailable,
    unknown: Boolean(card.unavailable),
    detail: card.unavailable
      ? "確認できません"
      : card.done
        ? (card.key === "attendance" ? (thAttendanceToday?.clockIn ? `${thAttendanceToday.clockIn} 出勤` : "記録済み") : "提出済み")
        : (card.key === "attendance" ? "未登録" : "未提出"),
  }));

  // 研修がない日の「いま抱えているもの」。**0件のものは並べない**（無いものの説明に場所を使わない）。
  const thOpenItems = [];
    ...
    <PrismPage className="max-w-full overflow-x-hidden" style={{ gap: 16 }}>
      ...
      <PrismCard className="overflow-hidden p-0">
        <div className="grid lg:grid-cols-[1fr_330px]">
          <div className="min-w-0 p-6 sm:px-7">
            <div className="flex flex-wrap items-center gap-2">
              <span
                className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11.5px] font-bold"
                style={thNoTrainingToday
                  ? { background: PRISM.neutralSubtle, color: PRISM.sub }
                  : { background: PRISM.accentSubtle, color: PRISM.accent }}
              >
                <Clock size={12} />
                {thNoTrainingToday ? formatTrainingDate(thToday) || thToday : "研修日"}
              </span>
              <span className="text-[11.5px]" style={{ color: PRISM.mut }}>
                {[!thNoTrainingToday ? formatTrainingDate(thToday) || thToday : "", thPrimaryCourse?.name].filter(Boolean).join(" ・ ")}
              </span>
            </div>

            <h1 className="mt-3.5 text-[26px] font-extrabold leading-[1.28] sm:text-[27px]" style={{ color: PRISM.ink, letterSpacing: "-0.035em" }}>
              {thHeroTitle}
            </h1>
            ...
            {!thLoading && (
              <div className="mt-5 flex flex-wrap gap-2">
                <Btn onClick={thHeroPrimaryOnClick}>{thHeroPrimaryLabel}</Btn>
                <Btn kind="ghost" onClick={() => go("curriculum")}>{thHeroSecondaryLabel}</Btn>
              </div>
            )}
          </div>

          {/* 右：今日の必須（研修日）／いま抱えているもの（研修がない日） */}
          <div
            className="border-t p-6 sm:px-7 lg:border-l lg:border-t-0"
            style={{ borderColor: PRISM.line, background: `linear-gradient(160deg, ${PRISM.accentSubtle} 0%, ${PRISM.surface} 62%)` }}
          >
            {thLoading ? <SkeletonRows rows={3} /> : thNoTrainingToday ? (
              <>
                <div className="text-[11.5px] font-extrabold" style={{ color: PRISM.mut, letterSpacing: "0.04em" }}>いま抱えているもの</div>
                <div className="mt-3 grid gap-2">
                  {thOpenItems.length ? thOpenItems.map(item => (
                    <StatusBoardStat key={item.key} tone={item.tone} value={item.value} unit={item.unit} label={item.label} />
                  )) : (
                    <div className="rounded-[14px] px-[15px] py-4 text-[12.5px] leading-relaxed" style={{ background: PRISM.surface, color: PRISM.sub }}>
                      残っているものはありません。
                    </div>
                  )}
                </div>
              ...
              <>
                <div className="text-[11.5px] font-extrabold" style={{ color: PRISM.mut, letterSpacing: "0.04em" }}>今日の必須</div>
                <div className="mt-3.5 flex items-baseline gap-2">
                  <span className="text-[40px] font-extrabold leading-none tabular-nums" style={{ color: PRISM.accent, letterSpacing: "-0.04em" }}>
                    {thTodayProgressAvailable ? thTodayCompletedCount : "—"}
                  </span>
                  <span className="text-[15px] font-bold" style={{ color: PRISM.mut }}>
                    / {thTodayProgressAvailable ? thTodayRequiredCount : 2} 完了
                  </span>
                </div>
                <div className="mt-3.5 grid gap-2.5">
                  {thRequiredToday.map(item => (
                    <div key={item.key} className="flex items-center gap-2.5">
                      {item.done ? (
                        <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full" style={{ background: PRISM.ok, color: "#fff" }}>
                          <Check size={11} strokeWidth={3.4} />
                        </span>
                      ) : (
                        <span className="h-5 w-5 shrink-0 rounded-full" style={{ border: `2px dashed ${item.unknown ? PRISM.mut : PRISM.warn}` }} />
                      )}
                      <span className="text-[12.5px]" style={{ color: item.done ? PRISM.sub : PRISM.ink, fontWeight: item.done ? 600 : 700 }}>
                        {item.label}
                        {item.detail && <span className="ml-1 font-normal" style={{ color: item.done ? PRISM.sub : PRISM.warn }}>{item.detail}</span>}
                      </span>
                    </div>
                  ))}
                </div>
                {thUnsubmittedTestCount > 0 && (
                  <div className="mt-4 border-t pt-3.5 text-[11.5px] leading-relaxed" style={{ borderColor: PRISM.line, color: PRISM.sub }}>
                    未提出の研修テストが<strong style={{ color: PRISM.warn }}>{thUnsubmittedTestCount}件</strong>あります
                  </div>
                )}
              </>
            )}
...
const testQuestionsOf = (t) => {
  ...
  return raw.map((q, i) => {
    ...
    return {
      ...
      // 「教材のどこを見ればよいか」。**編集して保存し直しても消さない**
      // （消えると、教材を差し替えたときの点検で戻り先が分からなくなる）
...
function TestBuilder({ back, focus, student, onSaved, initialTest = null, duplicate = false, mode = "ai" }) {
  ...
  // 教材と照合した結果。設問の並び順（0始まり）をキーにする（2026-09-18）
  ...
  /* ---- 教材と照合して点検する（2026-09-18）----
     スライドが差し替わった・内容が直ったときに、テストを作り直さずに済ませるための仕組み。
     **AIは判定と修正案を出すだけで、設問には触らない。** 反映は下の「この修正を反映」を
     押したときだけ行う。APIのタイムアウトが29秒なので、4問ずつに分けて順に投げる。 */
  const REVIEW_BATCH = 4;
  function reviewQuestionPayload(q, i) {
    const isChoice = (q.type || "choice") === "choice" || q.type === "trueFalse";
    return {
      number: i + 1,
      type: q.type || "choice",
      answerMode: answerModeOf(q),
      question: q.q || "",
      choices: isChoice && Array.isArray(q.a) ? q.a.filter(c => String(c ?? "").trim()) : [],
  ...
  async function reviewAgainstMaterial() {
    if (reviewBusy) return;
    if (!sourcePages.length) {
      setReviewErr("先に教材（PDF）とページ範囲を指定して、本文を読み取ってください。読み取ったページと突き合わせて点検します。");
      return;
    }
    const targets = qs.map((q, i) => ({ q, i })).filter(x => (x.q.q || "").trim());
    if (!targets.length) {
      setReviewErr("点検できる設問がありません。");
      return;
    }
    setReviewErr(""); setReviewedNote(""); setReviews({});
    setReviewBusy(true);
    setReviewProgress({ done: 0, total: targets.length });
    const collected = {};
    try {
      for (let start = 0; start < targets.length; start += REVIEW_BATCH) {
        const batch = targets.slice(start, start + REVIEW_BATCH);
        const data = await apiPost("/ai/tests/review", {
          courseId,
          courseName: selectedCourse?.name || "",
          testTitle: name.trim(),
          materialId,
          materialTitle: selectedMaterial?.title || "",
          instruction: scope.trim(),
          sourcePages,
          questions: batch.map(x => reviewQuestionPayload(x.q, x.i)),
        });
        const got = Array.isArray(data?.reviews) ? data.reviews : [];
        // 返ってきた number（＝設問の並び順）で戻す。欠けた分は「判定なし」のまま残す
        got.forEach((r, k) => {
          const idx = Number.isInteger(Number(r?.number)) ? Number(r.number) - 1 : (batch[k]?.i ?? -1);
          if (idx >= 0 && idx < qs.length) collected[idx] = { ...r, applied: false };
        });
        setReviews({ ...collected });
        setReviewProgress({ done: Math.min(start + batch.length, targets.length), total: targets.length });
      }
      const judged = Object.values(collected);
      const fix = judged.filter(r => r.verdict === "fix").length;
      const outdated = judged.filter(r => r.verdict === "outdated").length;
      const missing = targets.length - judged.length;
      setReviewedNote(
        (fix || outdated
          ? `要確認：修正案あり${fix}問／教材に見当たらない${outdated}問。中身を見てから反映してください。`
          : "読み取ったページと食い違う問題は見つかりませんでした。")
        + (missing ? `（${missing}問はAIから判定が返りませんでした。もう一度点検してください。）` : "")
      );
    } catch (e) {
      const debug = [e?.errorCode, e?.errorMessage, e?.hint].filter(Boolean).join(" / ");
      setReviewErr("点検に失敗しました。" + (debug || e?.message || "") + " 設問は変更していません。");
    } finally {
      setReviewBusy(false);
    }
  }
  /* 修正案を1問に反映する。**patchに入っている項目だけ**差し替え、他はそのまま残す */
  function applyReview(i) {
    const r = reviews[i];
    if (!r || r.verdict === "outdated") return;
    const patch = r.patch || {};
    setQs(list => list.map((row, j) => {
      if (j !== i) return row;
      const isChoice = (row.type || "choice") === "choice" || row.type === "trueFalse";
      const next = { ...row, w: "教材に合わせて修正" };
      if (patch.question) next.q = patch.question;
      if (isChoice && Array.isArray(patch.choices) && patch.choices.length) next.a = patch.choices;
      if (isChoice && Number.isInteger(patch.correctIndex)) next.correctIndex = patch.correctIndex;
      // 選択肢か正解が変わったら、保存用の answer も新しい選択肢に合わせ直す
      if (isChoice && (patch.choices || Number.isInteger(patch.correctIndex))) {
        next.answer = (next.a || [])[next.correctIndex] || "";
      }
      if (!isChoice && patch.modelAnswer) next.modelAnswer = patch.modelAnswer;
      if (patch.explanation) next.explanation = patch.explanation;
      if (patch.wrongReason) next.wrongReason = patch.wrongReason;
      if (patch.reviewPoint) next.reviewPoint = patch.reviewPoint;
      // 戻り先のページ。スライドが増減してズレた分をここで直す
      if (r.pageMoved && Number(r.sourcePage) > 0) {
        next.sourcePage = Number(r.sourcePage);
        next.sourceMaterialId = materialId || row.sourceMaterialId || "";
      }
      return next;
    }));
  ...
  return (
    <div>
      ...
      {/* 教材（PDF）の本文を読み取る。**ブラウザで読み取ってから渡す**（サーバはPDFを開けない）。
          問題を作るときと、あとから教材と照合して点検するときの両方で使う */}
      <Card className="mb-4 p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <div className="text-xs" style={{ color: T.textMuted }}>
              {aiMode
                ? "問題は、ここで読み取ったページからだけ作ります。ページ範囲は空欄なら全ページです。設問には「何ページを見ればよいか」が付きます。"
                : "教材を差し替えたときは、ここで読み取ってから下の「教材と照合して点検」を使います。ページ範囲は空欄なら全ページです。"}
            </div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
      ...
      {/* 教材と照合して点検する。**テストは作り直さない。** 食い違うところだけ拾って、
          反映するかどうかは問ごとに講師が決める（2026-09-18） */}
      {qs.length > 0 && (
        <Card className="mb-4 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-sm font-bold" style={{ color: T.textPrimary }}><RefreshCw size={15} style={{ color: T.accent }} />教材と照合して点検</div>
              <p className="mt-1 text-xs leading-relaxed" style={{ color: T.textMuted }}>
                上で読み取ったページと、今の{qs.length}問を突き合わせます。<b>問うている内容は変えません。</b>
                教材と食い違うところ・教材から無くなったところ・ページ番号のズレだけを拾って、修正案として出します。
                設問が書き換わるのは、下の各問で<b>「この修正を反映」</b>を押したときだけです。
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          {!sourcePages.length && (
            <div className="mt-3 rounded-lg px-3 py-2 text-xs" style={{ background: T.bgBase, color: T.textMuted }}>
              先に上で教材とページ範囲を指定して「このページを読み取る」を押してください。差し替え後のページを読み取ると、その内容と突き合わせます。
            </div>
          )}
          {reviewList.length > 0 && (
            <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
              <Badge tone="green">教材と合っている {reviewOkCount}問</Badge>
              {reviewFixCount > 0 && <Badge tone="amber">修正案あり {reviewFixCount}問</Badge>}
              {reviewOutdatedCount > 0 && <Badge tone="red">教材に見当たらない {reviewOutdatedCount}問</Badge>}
            </div>
          )}
          {reviewedNote && <div className="mt-2 rounded-lg px-3 py-2 text-xs" style={{ background: T.bgBase, color: T.textMuted }}>{reviewedNote}</div>}
          {reviewErr && <div className="mt-2 rounded-lg px-3 py-2 text-xs font-semibold" style={{ background: T.warningSubtle, color: T.warning }}>{reviewErr}</div>}
        </Card>
      )}
      ...
      {qs.map((item, i) => (
        <Card key={i} className="mb-3 p-5">
          <div className="mb-2 flex flex-wrap items-center gap-2"><span className="font-bold" style={{ color: T.accent }}>Q{i + 1}</span>
            ...
            {item.topic && <Badge tone="cyan">{item.topic}</Badge>}
            {Number(item.sourcePage) > 0 && <Badge tone="muted">教材 {item.sourcePage}ページ</Badge>}</div>
          {/* 教材と照合した結果。**ここでは何も書き換わっていない。**「反映」を押して初めて上の欄が変わる */}
          {reviews[i] && (
            reviews[i].verdict === "ok" ? (
              <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg px-3 py-2 text-xs" style={{ background: T.successSubtle, color: T.success }}>
                <CheckCircle2 size={13} />教材と合っています{Number(reviews[i].sourcePage) > 0 ? `（${reviews[i].sourcePage}ページ）` : ""}
                {reviews[i].pageMoved && <span style={{ color: T.warning }}>・戻り先のページが{item.sourcePage || "未設定"}→{reviews[i].sourcePage}に変わっています</span>}
                {reviews[i].pageMoved && !reviews[i].applied && <Btn kind="ghost" size="sm" onClick={() => applyReview(i)}>ページ番号を直す</Btn>}
              </div>
            ) : reviews[i].verdict === "outdated" ? (
              <div className="mb-3 rounded-lg px-3 py-2 text-xs" style={{ background: T.dangerSubtle, color: T.danger }}>
                <div className="flex items-center gap-2 font-bold"><AlertTriangle size={13} />この問題の内容が、読み取ったページに見当たりません</div>
                {reviews[i].reason && <div className="mt-1 leading-relaxed">{reviews[i].reason}</div>}
                <div className="mt-1 leading-relaxed" style={{ color: T.textMuted }}>消すか、別の内容に差し替えるかは講師が決めてください。ページ範囲の指定が狭すぎる場合もあります。</div>
              </div>
            ) : (
              <div className="mb-3 rounded-lg px-3 py-2.5 text-xs" style={{ background: T.warningSubtle, border: `1px solid ${T.border}` }}>
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="flex items-center gap-2 font-bold" style={{ color: T.warning }}><AlertTriangle size={13} />教材と食い違っています</div>
                  {reviews[i].applied
                    ? <Badge tone="green">反映しました</Badge>
                    : <Btn size="sm" icon={CheckCircle2} onClick={() => applyReview(i)}>この修正を反映</Btn>}
                </div>
                {reviews[i].reason && <div className="mt-1 leading-relaxed" style={{ color: T.textPrimary }}>{reviews[i].reason}</div>}
                {!reviews[i].applied && (
                  <div className="mt-2 space-y-1.5" style={{ color: T.textSecondary }}>
                    {reviews[i].patch?.question && <div><b>設問</b>：{reviews[i].patch.question}</div>}
                    {Array.isArray(reviews[i].patch?.choices) && (
                      <div><b>選択肢</b>：<ul className="mt-0.5 list-disc pl-5">{reviews[i].patch.choices.map((c, ci) => <li key={ci}>{c}</li>)}</ul></div>
                    )}
                    {Number.isInteger(reviews[i].patch?.correctIndex) && <div><b>正解</b>：選択肢 {reviews[i].patch.correctIndex + 1}</div>}
                    {reviews[i].patch?.modelAnswer && <div><b>模範解答</b>：{reviews[i].patch.modelAnswer}</div>}
                    {reviews[i].patch?.explanation && <div><b>解説</b>：{reviews[i].patch.explanation}</div>}
                    {reviews[i].patch?.wrongReason && <div><b>よくある誤答理由</b>：{reviews[i].patch.wrongReason}</div>}
                    {reviews[i].patch?.reviewPoint && <div><b>復習ポイント</b>：{reviews[i].patch.reviewPoint}</div>}
                    {reviews[i].pageMoved && <div><b>戻り先のページ</b>：{item.sourcePage || "未設定"} → {reviews[i].sourcePage}</div>}
                  </div>
                )}
              </div>
            )
...
function ClientHome({ openKarte, go, goProduct }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiGet("/trainees").catch(() => []),
      apiGet("/companies").catch(() => []),
      apiGet(`/dashboard/client?date=${clientDate}`).catch(() => null),
    ])
      .then(([ts, companies, dash]) => {
        if (!alive) return;
        ...
        const visibleTrainees = apiTrainees.length
          ? apiTrainees.map(t => {
              ...
              return { ...t, course: t.course || dashboardTrainee?.courseId || "" };
            })
          : dashboardTrainees.map(t => ({ userId: t.traineeId, name: t.name, course: t.courseId || "" }));
  ...
  /* 企業担当ホーム（2026-09-10 作り直し）。「気になる受講生」はやめた。
     何をもって気になるとするかの決めごとが増えるほど出てくる人が変わり、信用されなくなる。
     出欠と日報という事実だけを出し、誰を気にするかは見た方が決める。 */
  const clientTodayTrainees = clientDashTrainees.filter(t => t.dayStatus === "training_day");
  const clientIsTrainingDay = clientTodayTrainees.length > 0;
  // 打刻がない・欠席・遅刻の方だけを出す。順調な方の名前を並べても見る用がない。
  const clientAttendanceIssues = clientTodayTrainees.filter(t => t.attendanceStatus !== "completed" && t.attendanceStatus !== "working");
  const clientAttendanceOk = clientTodayTrainees.length - clientAttendanceIssues.length;
  const clientAttendanceLabel = t => (
    t.attendanceStatus === "not_clocked_in" ? "まだ打刻がありません"
      : t.attendanceStatus === "absent" ? "欠席"
      : t.attendanceStatus === "late" ? "遅刻"
      : t.attendanceStatus === "early_leave" ? "早退"
      : "確認できません"
  );
  const clientReportPending = clientTodayTrainees.filter(t => t.reportStatus === "not_submitted");
  const clientReportSubmitted = clientTodayTrainees.length - clientReportPending.length;
  const clientProgress = clientDash?.trainingProgress || null;
  const clientProgressAvailable = clientProgress?.available === true && clientProgress.total > 0;
  const clientUpcoming = Array.isArray(clientDash?.upcomingTrainingDays) ? clientDash.upcomingTrainingDays : [];
  const clientUpcomingAvailable = clientDash?.upcomingTrainingDaysAvailable !== false;

  const clientHeadingTitle = clientLoading
    ? "本日の状況を確認しています"
    : !clientDash
      ? "本日の状況を確認できません"
      : !clientIsTrainingDay
        ? "本日は研修日ではありません"
        : clientAttendanceIssues.length || clientReportPending.length
          ? `本日は${clientTodayTrainees.length}名が研修中 — 出欠${clientAttendanceIssues.length}件・日報${clientReportPending.length}件が未確認です`
          : `本日は${clientTodayTrainees.length}名が研修中 — 出欠も日報も揃っています`;
  const clientHeadingDescription = !clientDash
    ? "最新の状況を取得できませんでした。時間をおいて再度お試しください。"
    : !clientIsTrainingDay
      ? "本日は研修の予定がありません。この先の予定と、これまでの記録が確認できます。"
      : "出欠と日報は記録をそのまま出しています。気になる方がいれば、その場から個別の状況へ進めます。";

  return (
    <PrismPage>
      <PrismHomeHeading
        eyebrow={`${homeDateLabel(clientDate)} ・ ${clientCompanyName || "自社"}`}
        title={clientHeadingTitle}
        description={clientHeadingDescription}
        action={<Btn onClick={() => go && go("trainees")}>受講生の状況を見る</Btn>}
      />

      {clientErr && <PrismErrorRetryCard message={clientErr} onRetry={() => setClientReloadKey(v => v + 1)} />}

      <div className="grid gap-3 sm:grid-cols-3">
        <PrismKpiCard icon={Users} tone="accent" label="自社の受講生" value={clientTrainees.length} unit="名"
          detail={clientIsTrainingDay ? `本日研修中 ${clientTodayTrainees.length}名` : "本日は研修日ではありません"} />
        <PrismKpiCard icon={Clock} tone={clientAttendanceIssues.length ? "warn" : "ok"} label="本日の出勤"
          value={clientIsTrainingDay ? clientAttendanceOk : "—"} unit={clientIsTrainingDay ? `/ ${clientTodayTrainees.length}名` : ""}
          detail={clientIsTrainingDay ? (clientAttendanceIssues.length ? `打刻がない方が${clientAttendanceIssues.length}名います` : "全員の打刻が揃っています") : "本日は対象がありません"} />
        <PrismKpiCard icon={Calendar} tone="teal" label="研修の進み"
          value={clientProgressAvailable ? clientProgress.done : "—"} unit={clientProgressAvailable ? `/ ${clientProgress.total}日` : ""}
          detail={clientProgressAvailable ? `全体の${Math.round((clientProgress.done / clientProgress.total) * 100)}%が終わりました` : "研修日程を確認できません"} />
      </div>

      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)]">
        <div className="flex min-w-0 flex-col gap-3">
          <TrainingHomePanel title="本日の出欠" meta={clientIsTrainingDay ? `${clientAttendanceOk} / ${clientTodayTrainees.length}名 出勤済み` : ""}>
            {clientLoading ? <SkeletonRows rows={2} /> : !clientDash ? (
              <div className="py-2 text-[12.5px]" style={{ color: PRISM.sub }}>出欠を確認できませんでした。</div>
            ) : !clientIsTrainingDay ? (
              <div className="py-2 text-[12.5px]" style={{ color: PRISM.sub }}>本日は研修日ではないため、出欠の対象がありません。</div>
            ) : !clientAttendanceIssues.length ? (
              <TrainingHomePanelRow icon={CheckCircle2} tone="ok" label="全員の打刻が揃っています" sub={`${clientTodayTrainees.length}名`} actionLabel="一覧" onAction={() => go && go("attendance")} />
            ) : (
              <>
                {clientAttendanceIssues.slice(0, 6).map(t => (
                  <TrainingHomePanelRow key={t.traineeId} icon={Clock} tone={t.attendanceStatus === "not_clocked_in" || t.attendanceStatus === "absent" ? "bad" : "warn"}
                    label={clientName(t)} sub={clientAttendanceLabel(t)} actionLabel="状況" onAction={() => openKarte && openKarte({ userId: t.traineeId, name: t.name })} />
                ))}
                {clientAttendanceOk > 0 && (
                  <TrainingHomePanelRow icon={CheckCircle2} tone="ok" label={`ほか${clientAttendanceOk}名は出勤済み`} sub="打刻された時刻は勤怠で確認できます" actionLabel="一覧" onAction={() => go && go("attendance")} />
                )}
              </>
            )}
          </TrainingHomePanel>

          <TrainingHomePanel title="日報の提出" meta={clientIsTrainingDay ? `${clientDate.replace(/-/g, "/")}分` : ""}>
            {clientLoading ? <SkeletonRows rows={2} /> : !clientDash ? (
              <div className="py-2 text-[12.5px]" style={{ color: PRISM.sub }}>日報の提出状況を確認できませんでした。</div>
            ) : !clientIsTrainingDay ? (
              <div className="py-2 text-[12.5px]" style={{ color: PRISM.sub }}>本日は研修日ではないため、日報の対象がありません。</div>
            ) : (
              <>
                <TrainingHomePanelRow icon={NotebookPen} tone={clientReportPending.length ? "warn" : "ok"}
                  label={`提出 ${clientReportSubmitted}名 ／ 未提出 ${clientReportPending.length}名`}
                  sub={clientReportPending.length ? clientReportPending.slice(0, 3).map(clientName).join("・") + (clientReportPending.length > 3 ? " ほか" : "") : "全員が提出しています"}
                  actionLabel="日報" onAction={() => go && go("reports")} />
                <TrainingHomePanelRow icon={BookOpen} tone="ai" label="研修の内容" sub="今日どんな単元を進めているかを確認できます" actionLabel="確認" onAction={() => go && go("curriculum")} />
              </>
            )}
          </TrainingHomePanel>
        </div>
        ...
        <div className="flex min-w-0 flex-col gap-3">
          <TrainingHomePanel title="この先の予定">
            {clientLoading ? <SkeletonRows rows={2} /> : !clientUpcoming.length ? (
              <div className="py-2 text-[12.5px]" style={{ color: PRISM.sub }}>
                {clientUpcomingAvailable ? "この先の研修日はまだ登録されていません。" : "この先の日程を確認できませんでした。"}
              </div>
            ) : clientUpcoming.map(item => (
              <TrainingHomePanelRow key={`${item.date}:${item.courseId}`} icon={Calendar} tone="accent"
                label={homeDateLabel(item.date)} sub={item.title || item.courseName || "内容はカリキュラムで確認できます"}
      ...
      {!clientLoading && !clientIsTrainingDay && (
        <PrismCard className="flex flex-wrap items-center justify-between gap-4 p-5">
          <div className="min-w-[240px] flex-1">
            <div className="text-[13.5px] font-bold" style={{ color: PRISM.ink }}>研修がない日に確認できること</div>
            <p className="mt-1.5 text-[12.5px] leading-relaxed" style={{ color: PRISM.sub }}>
              これまでの日報や勤怠の記録、カリキュラムの進み具合が確認できます。
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Btn size="sm" kind="soft" onClick={() => go && go("reports")}>これまでの日報</Btn>
              <Btn size="sm" kind="ghost" onClick={() => go && go("curriculum")}>カリキュラム</Btn>
            </div>
          </div>
          <div className="hidden shrink-0 md:block" aria-hidden="true"><ClientHomeIllustration /></div>
        </PrismCard>
      )}
    </PrismPage>
  );
